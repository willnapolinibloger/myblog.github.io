...
1. Вспомни ситуацию, где ты ругал себя *(например: «Опять проспал! Я безвольный тряпка!»)*
2. Найди «часть», которая это говорит *(например: «Это мой Внутренний Перфекционист злится»)*
3. Спроси её: **«Что ты пытаешься для меня сделать?»** *(Может, защитить от провала?)*
4. Поблагодари и предложи альтернативу *(например: «Спасибо, что хочешь, чтобы я был лучше. Давай просто встану на 10 минут раньше?»)*

> **Эффект:** как от медитации, только приятнее ❤️
{% endcapture %}

{% include modern-card.html 
   title="«Перепиши свой внутренний диалог»" 
...
{% include modern-card.html 
  ...
  {% capture tips_content %}
- Используйте <mark>яркие цвета</mark> для привлечения внимания
- Добавляйте интерактивные элементы
- Не забывайте про мобильную адаптацию
- Тестируйте в разных браузерах

> **Совет:** Всегда думайте о пользователе в первую очередь!
{% endcapture %}

{% include modern-card.html 
   title="5 советов по веб-дизайну"
   ...
   link="https://t.me/your_special_channel" %}

   ---

   {% include cta-box.html 
   title="🚀 Курс по продуктивности" 
   subtitle="«21 день к новой жизни»"
   description="получи доступ к эксклюзивным материалам"
   list_items="Освой тайм-менеджмент|Повысь концентрацию|Достигай больше целей|Избавься от прокрастинации"
   link="https://t.me/productivity_course"
   button_text="🎯 Начать курс"
   note="Доступ открыт только до конца месяца" %}

   ---

   {% include cta-box.html 
   title="📖 Рекомендация книги" 
   subtitle="«Атомные привычки»"
   description="узнай, как маленькие изменения приводят к большим результатам"
   list_items="Создавай полезные привычки|Избавляйся от вредных|Трансформируй свою жизнь|Достигай поставленных целей"
   link="https://t.me/book_summaries"
   button_text="📚 Скачать краткое изложение"
   note="Бесплатно для подписчиков канала" %}
